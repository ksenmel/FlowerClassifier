...
def predict_mode(args):
    ...
    print("Classes probability distribution for image:")
    print(probs)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Parse parameters for the flower classification application")
    parser.add_argument("--mode", help="Mode of the application: 'train' for training, 'predict' for evaluating",
                        choices=["train", "predict"], default="predict")
    parser.add_argument("-i", "--img_path", help="Path to the input image with flower to classify",
                        default="tests/0.jpeg")
    parser.add_argument("-m", "--model_path", help="Path to classification model")
    parser.add_argument("-d", "--dataset_path", help="Path to the dataset for training",
                        default="models/v1FlowerClassifier_model.pkl")
    parser.add_argument("-s", "--path_to_save", help="path to save classification model")
